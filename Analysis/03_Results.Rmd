...
db_vx_pfro                <-  readRDS("forecast_intervention/db_frcst_bestmodels_vx_pfro.rds")
...
#3)
  ...
  fitted_flpm_pfro<-readRDS("fit_preintervention/fitted_values_flpm_pfro2024v11_3.rds")    %>% 
                  select(.model_desc,date,.prediction,id) %>% 
                  rename(.value = .prediction,
                         .index =  date) %>% 
                  mutate(.key   = "fitted")
...
###############  
# 4
 db_flpm_pfro.train<-  db_flpm_pfro                                                               %>% # prediccion + actual
                     rbind(fitted_flpm_pfro)                                                    %>% # fitted values
                     mutate(.model_desc = str_replace(.model_desc, ".*PROPHET.*", "PROPHET"),
                            .model_desc = str_replace(.model_desc, ".*NNAR.*", "NNAR"),
                            .model_desc = str_replace(.model_desc, "2_A.*", "ARIMAXG"),
                            .model_desc = str_replace(.model_desc, "3_R.*", "ARIMA"),
...
#3)
  ...
  fitted_vx_mcero<-readRDS("fit_preintervention/fitted_values_vx_mcero2024v11_3.rds")    %>% 
                  select(.model_desc,date,.prediction,id) %>% 
                  rename(.value = .prediction,
                         .index =  date) %>% 
                  mutate(.key   = "fitted")
...
############  
# 4
 db_vx_mcero.train<-  db_vx_mcero                                                               %>% # prediccion + actual
                     rbind(fitted_vx_mcero)                                                    %>% # fitted values
                     mutate(.model_desc = str_replace(.model_desc, ".*PROPHET.*", "PROPHET"),
                            .model_desc = str_replace(.model_desc, ".*NNAR.*", "NNAR"),
                            .model_desc = str_replace(.model_desc, "2_A.*", "ARIMAXG"),
                            .model_desc = str_replace(.model_desc, "3_R.*", "ARIMA"),
...
#3)
  ...
  fitted_flpm_mcero<-readRDS("fit_preintervention/fitted_values_flpm_mcero2024v11_3.rds")    %>% 
                  select(.model_desc,date,.prediction,id) %>% 
                  rename(.value = .prediction,
                         .index =  date) %>% 
                  mutate(.key   = "fitted")
...
################  
# 4
 db_flpm_mcero.train<-db_flpm_mcero                                                               %>% # prediccion + actual
                     rbind(fitted_flpm_mcero)                                                    %>% # fitted values
                     mutate(.model_desc = str_replace(.model_desc, ".*PROPHET.*", "PROPHET"),
                            .model_desc = str_replace(.model_desc, ".*NNAR.*", "NNAR"),
                            .model_desc = str_replace(.model_desc, "2_A.*", "ARIMAXG"),
                            .model_desc = str_replace(.model_desc, "3_R.*", "ARIMA"),
