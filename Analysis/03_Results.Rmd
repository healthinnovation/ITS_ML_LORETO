...
## Figure 1 - Figure S1: Time series plot by prov {.tabset}
...
(Falciparum.ts.plot<- db_flpm_pfro                                                                   %>% 
                  rbind(fitted_flpm_pfro)                                                        %>% 
                  mutate(.model_desc = str_replace(.model_desc, ".*PROPHET.*", "PROPHET"),
                          .model_desc = str_replace(.model_desc, ".*NNAR.*", "NNAR"),
                          .model_desc = str_replace(.model_desc, "2_A.*", "ARIMAXG"),
...
## Figure 2 - Figure S2: Spatial distribution{.tabset}
...
```{r,messsage=F,warning=F,fig.width=7,fig.height=10}


library(ggpubr)
figure2.map <- ggarrange(Vivax.map, Falciparum.map,
                    ncol = 1, nrow = 2)
figure2.map
...
(Falciparum.mc.map<- db.shp.mcero                                                                %>% 
                  ggplot()                                                                        +
                  geom_sf(aes(fill=dif_pht))                                                      +
                  scale_fill_gradient2( low = "gold3",  
                                        mid = "white", 
...
library(ggpubr)
figure2.mc.map <- ggarrange(Vivax.mc.map, Falciparum.mc.map,
                    ncol = 1, nrow = 2)
figure2.mc.map

```

# Figure S3-S4: BOXPLOT by specie
...
# Figure S3-S4: BOXPLOT by intervention
